...
type SuperadminPermissionsEditProps = {
  superadmin: AccountPermissions
  refetch: () => void
}

const updateSuperadminPermissionsSchema = zod.object({
  id: zod.uuid(),
...
type UpdateSuperadminPermissionsValues = zod.infer<typeof updateSuperadminPermissionsSchema>
...
export default function SuperadminPermissionsEdit({ superadmin, refetch }: SuperadminPermissionsEditProps) {
  const fullName = [superadmin.firstName, superadmin.lastName].filter(Boolean).join(' ')
  const moduleEntries = Object.entries(groupPermissionsByModule(superadmin.permissions))
  const form = useForm<UpdateSuperadminPermissionsValues>({
    id: superadmin.id,
    permissions: superadmin.permissions,
  }, {
    onSubmit: (body) => permissionsService.put(body),
    schema: updateSuperadminPermissionsSchema,
  ...
  return (
    <CustomPage
      title="Permisos del Super Admin"
      description="Consulta y modifica los permisos asignados a la cuenta"
      goBackPath="/superadmins"
    >
      <div className="space-y-6">
        <CustomPageContainer className="p-6">
          <div className="flex items-center gap-4">
            <Avatar className="size-16 border border-gray-200 dark:border-gray-700">
              <AvatarFallback className="bg-linear-to-br from-purple-400 to-purple-600 text-lg font-semibold text-white">
                {getInitials(superadmin.firstName, superadmin.lastName) || 'S'}
              </AvatarFallback>
            </Avatar>
            <div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary">
              {superadmin.permissions.length} {superadmin.permissions.length === 1 ? 'permiso' : 'permisos'}
            </Badge>
          </div>
        ...
        <CustomPageContainer className="p-6">
          <Form onSubmit={form.handleSubmit} className="space-y-6">
            ...
            <FormPermissionsEditor
              ...
              name="permissions"
              label="Permisos del Super Admin"
