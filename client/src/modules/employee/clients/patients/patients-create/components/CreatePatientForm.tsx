...
import { patientSchema, type PatientValues } from '@/modules/employee/clients/patients/patients.schema'

type CreatePatientFormProps = {
  client: Client
}

const defaultValues = { name: '', breed: '', sex: null, birthDate: null } as PatientValues

export default function CreatePatientForm({ client }: CreatePatientFormProps) {
  const patientsPath = `/clients/${client.id}/patients`
  const form = useForm(defaultValues, {
    onSubmit: (body) => clientsPatientsService.post({ ...body, client: client.id }),
    schema: patientSchema,
    successMessage: 'Mascota registrada correctamente',
    redirectTo: patientsPath,
  })

  return (
    <CustomPage title="Nueva mascota" description={`Registra una mascota de ${client.name}.`} goBackPath={patientsPath}>
