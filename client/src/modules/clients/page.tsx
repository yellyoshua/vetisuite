...
import { inputStyle, isOpenVisit, money, T } from "@/lib/constants";
  ...
  return (
    <CustomPage title="Clientes y Pacientes" description="Una sola fuente de verdad: cada dueño vinculado a sus mascotas."
      ...
      <div className="flex items-center gap-2 mb-1" style={{ ...inputStyle, padding: "8px 11px", maxWidth: 480 }}>
        ...
        <input type="search" aria-label="Buscar cliente" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar cliente (nombre, teléfono, correo)…"
      ...
      {!loading && res.rows.map((c) => {
        ...
        const visit = s.visits.find((v) => v.clientId === c.id && isOpenVisit(v));
        return (
          <ResourceListItem key={c.id}
            ...
            actions={<>
              <Btn small kind="ghost" disabled={petCount === 0} onClick={() => { const fp = s.patients.find((p) => p.clientId === c.id)!.id; navigate(`/visits/edit/${s.openVisit(c.id, fp)}`); }}><ClipboardList size={13} /> Visita</Btn>
