...
import { FileText, FlaskConical, Stethoscope, Syringe } from "lucide-react";
import { F, isActiveRecord, T } from "@/lib/constants";
...
export default function ClinicShowPage() {
  ...
  const activeRecord = patientRecords.find(isActiveRecord);
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          {activeRecord && (
            <div className="flex gap-2 flex-wrap">
              ...
              <Btn small kind="ghost" onClick={() => navigate(`/clinic/prescription/${patient.id}`)}><FileText size={13} /> Emitir receta</Btn>
            </div>
          )}
          {!activeRecord && patientRecords.length > 0 && (
            <p style={{ fontSize: 12.5, color: T.sub }}>
              No hay consulta activa: los insumos y las recetas se cuelgan de una consulta abierta hoy. Abre una con “Nueva consulta”.
            </p>
          )}
          {patientRecords.map((r) => {
            const vet = s.vets.find((v) => v.id === r.vetId);
            return (
              <Card key={r.id} className="p-4" style={r.id === activeRecord?.id ? { borderColor: T.green } : {}}>
                <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                  ...
                  {r.id === activeRecord?.id && <Badge tone="green">Consulta activa</Badge>}
                </div>
